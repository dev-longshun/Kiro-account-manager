...
本项目为 Electron 桌面应用（electron-vite 构建），编译规范如下：

- 禁止通过命令行执行 `npm run dev` 或 `npm run build:*` 等长时间运行的构建/启动命令，提醒用户在终端手动执行
- 可以执行 `npm run typecheck`、`npm run lint` 等一次性检查命令
- 代码修改后提醒用户手动运行 `npm run dev`（在 `Kiro-account-manager/` 子目录下）验证效果
- 注意实际项目代码在 `Kiro-account-manager/Kiro-account-manager/` 路径下，`npm` 命令需在该子目录执行
